...
//News
try{
	URL query = new URL("http://localhost:8983/solr/WikiNews/select?q="+qURL+"&sort=timestamp+asc&wt=xml&indent=true&hl=true&hl.fl=text");
	URLConnection conn = query.openConnection();
	BufferedReader in = new BufferedReader(
			new InputStreamReader(conn.getInputStream()));
	//System.out.println("NEWS__________________________________________________________________");
	String inputline = null;
	while ((inputline = in.readLine()) != null) 
	{   inputline=inputline.replaceAll("<br>", "br!");
		if(!inputline.contains("<date")&&!inputline.contains("<str") && !inputline.contains("<arr name")&&!inputline.contains("</arr>")&&!inputline.contains("</lst>")&&!inputline.contains("<doc>")&&!inputline.contains("</doc>")&&!inputline.contains("<long name=")&&!inputline.contains("<response>")&&!inputline.contains("</response>")&&!inputline.contains("<lst")&&!inputline.contains("<int name=")&&!inputline.contains("</str>")&&!inputline.contains("<result name")&&!inputline.contains("</result>"))
		{
		inputline=inputline.replaceAll("<[^>]*>", "");
		inputline=inputline.replaceAll("<|>","");
		}
...
try {
				...
				public void endElement(String uri, String localName,
					...
					//	System.out.println("count_lst="+count_lst);
						...
						//System.out.println("LOL"+characters.toString());
				...
				public void characters(char ch[], int start, int length) throws SAXException {
			     ...
			     counter=0;
					for(int i=1; i<11;i++)
					{
						if(NewsText[i]!=null)
						{counter++;
						NewsText[i]=NewsText[i].replaceAll("$","");
						NewsText[i]=NewsText[i].replaceAll("quot","");
...
<font size="2">
<table border="1">
<tr height=><th><%out.println(Q);%>'s Wikipedia Page</th><th>News About "<%out.println(Q);%>"</th><th>Books About "<%out.println(Q);%>"</th><th>Quotes About "<%out.println(Q);%>"</th></tr>
<tr>
<!-- <td width="500" height="700"></td>  -->
<td align=left valign=top style=" table-layout:fixed; overflow:hidden;" width="600" height="700"><iframe width="600" height="700" id="first" src="http://www.wikipedia.com/wiki/<%out.println(qURL);%>"></iframe></td>
<td align=left valign=top style=" table-layout:fixed; overflow:hidden;" width="500px" height="700"><frame width="500" height="700">
<% if(NewsTitle[1]!=null)
{
for(int i=1;i<=10;i++){
	if(NewsTitle[i]!=null){
		out.println("<a href=\"http://en.wikinews.org/wiki/"+NewsTitle[i]+"\">"+NewsTitle[i]+"</a><br><font size=\"1\">"+NewsText[i]+"</font><br><br>");};}
}
else
	out.println("Sorry, couldn't find any news related to this person");
	 %>
</frame>
</td>
<td align=left valign=top style=" table-layout:fixed; overflow:hidden;" width="500px" height="700"><frame width="500" height="700">
<% if(BooksTitle[1]!=null)
{
	
for(int i=1;i<=10;i++){
	if(BooksTitle[i]!=null){
		out.println("<a href=\"http://en.wikibooks.org/wiki/"+BooksTitle[i]+"\">"+BooksTitle[i]+"</a><br><font size=\"1\">"+BooksText[i]+"</font><br><br>");};
	}
}
else
	out.println("Sorry, couldn't find any books related to this person");
%>
</frame>
</td>
<td align=left valign=top style=" table-layout:fixed; overflow:hidden;" width="500px" height="700"><frame width="500" height="700">
<%if(QuotesTitle[1]!=null)
{
for(int i=1;i<=10;i++){
	if(QuotesTitle[i]!=null){
		out.println("<a href=\"http://en.wikiquote.org/wiki/"+QuotesTitle[i]+"\">"+QuotesTitle[i]+"</a><br><font size=\"1\">"+QuotesText[i]+"</font><br><br>");};}
}
else
	out.println("Sorry, couldn't find any quotes related to this person");
	 %>
</frame>
</td>
</tr>
